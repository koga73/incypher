...
describe("--- CryptoProvider ---\n", function () {
	...
	it("encrypt", function () {
		...
		const ciphertext = InCypher.atob(InCypher.encrypt(deterministicIV, key, testPhrase));
	...
	it("decrypt", function () {
		...
		const deterministicIV = InCypher.deterministicIV(startIV, fixed, incremental);
		const key = InCypher.hash("mySecretKey");
		const ciphertext = InCypher.btoa("S1Evemh0bEZSa0RTbXUvYWRBME5zeDFGWFRwRzlGL2piZmZrcmVHOFU2aWFzUFlTT3lDWVdMWC82YnB1azJqOVZnV01TOWc0bmJVSURnUT0=");
		const plaintext = InCypher.decrypt(deterministicIV, key, ciphertext);
		expect(plaintext).equal(testPhrase);
	});
	it("hash | SHA-256", function () {
		const hash = InCypher.byteArrayToHex(InCypher.hash(testPhrase));
	...
	it("deterministicIV", function () {
		...
		const fixed = InCypher.deterministic32BitVal("fixedPhrase");
		const incremental = 73;
		const deterministicIV = InCypher.byteArrayToHex(InCypher.deterministicIV(startIV, fixed, incremental));
	...
	it("byteArrayToHex", function () {
		const hex = InCypher.byteArrayToHex(new Uint8Array([0x01, 0x02, 0x03, 0x04]));
		expect(hex).equal("01020304");
	});
	it("hexToByteArray", function () {
		const byteArray = InCypher.hexToByteArray("01020304");
	...
	it("atob", function () {
		const base64Encoded = InCypher.atob(testPhrase);
		expect(base64Encoded).equal("VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IGRvZw==");
	});
	it("btoa", function () {
		const unencoded = InCypher.btoa("VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IGRvZw");
